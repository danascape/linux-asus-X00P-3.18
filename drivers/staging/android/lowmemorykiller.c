...
#include <linux/slab.h>//liuxiong@wind-mobi.com 20180904 asus patch
...
//liuxiong@wind-mobi.com 20180904 asus patch begin
...
//liuxiong@wind-mobi.com 20180904 asus patch end
...
//static unsigned long lowmem_deathpending_timeout; //liuxiong@wind-mobi.com 20180904 asus patch
...
unsigned long time_out;//liuxiong@wind-mobi.com 20180904 asus patch
...
*/
	...
	if (ddr_size == 2) {
		...
		if (pressure >= 98) {
			...
			if (((other_free < lowmem_minfree[array_size - 1]) &&
				(other_file < vmpressure_file_min)) ||(other_free < 4096)) {//ASUS BSP ,if other_free less than 16M,no need to judge other_file
					atomic_set(&shift_adj, 1);
					trace_almk_vmpressure(pressure, other_free,
						other_file);
			}
		} else if (atomic_read(&shift_adj) && (other_free > 4096)) {//ASUS BSP ,if other_free less than 16M,no need to judge other_file
			/*
