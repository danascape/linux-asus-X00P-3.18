...
static void himax_get_thx_info(uint32_t* dataBuf)
{
	...
	mutual_key_dev_fail_upper = (uint8_t)dataBuf[10];	//30%
	mutual_key_dev_fail_lower = -(uint8_t)dataBuf[11];	//-30%
	...
	I("%s : noice_thx = %d \n", __func__, noice_thx);
	I("%s : mutual_key_dev_fail_upper = %d \n", __func__, mutual_key_dev_fail_upper);
	I("%s : mutual_key_dev_fail_lower = %d \n", __func__, mutual_key_dev_fail_lower);
...
static void himax_write_config_table(uint8_t type, uint32_t* dataBuf, int data_size)
{
	...
	if(type == 1) //FE
	{
		...
		{
			case 0x02:
				c8 = (uint8_t *)kzalloc(data_size * sizeof(uint8_t),GFP_KERNEL);
				memcpy(c8, writeBuf, data_size * sizeof(uint8_t));
				break;
			case 0x6C:
				c9 = (uint8_t *)kzalloc(data_size * sizeof(uint8_t),GFP_KERNEL);
				memcpy(c9, writeBuf, data_size * sizeof(uint8_t));
				break;
			case 0xE0:
				c10 = (uint8_t *)kzalloc(data_size * sizeof(uint8_t),GFP_KERNEL);
				memcpy(c10, writeBuf, data_size * sizeof(uint8_t));
				break;
			case 0xE6:
				c11 = (uint8_t *)kzalloc(data_size * sizeof(uint8_t),GFP_KERNEL);
	...
	else
	{
		switch(writeBuf[0])
		{
			case 0xE1:
				...
				break;
			case 0xBC:
				c2 = (uint8_t *)kzalloc(data_size * sizeof(uint8_t),GFP_KERNEL);
				memcpy(c2, writeBuf, data_size * sizeof(uint8_t));
				break;
			case 0xC5:
				c3 = (uint8_t *)kzalloc(data_size * sizeof(uint8_t),GFP_KERNEL);
				memcpy(c3, writeBuf, data_size * sizeof(uint8_t));
				break;
			case 0xC6:
				c4 = (uint8_t *)kzalloc(data_size * sizeof(uint8_t),GFP_KERNEL);
				memcpy(c4, writeBuf, data_size * sizeof(uint8_t));
				break;
			case 0xEC:
				c5 = (uint8_t *)kzalloc(data_size * sizeof(uint8_t),GFP_KERNEL);
				memcpy(c5, writeBuf, data_size * sizeof(uint8_t));
				break;
			case 0x3F:
				c6 = (uint8_t *)kzalloc(data_size * sizeof(uint8_t),GFP_KERNEL);
				memcpy(c6, writeBuf, data_size * sizeof(uint8_t));
				break;
			case 0xA4:
				c7 = (uint8_t *)kzalloc(data_size * sizeof(uint8_t),GFP_KERNEL);
	...
	switch(type)
		{
			case GOLDEN_MUTUAL_DATA_TYPE:
				...
				golden_mutual_baseC = (uint32_t *)kzalloc(ini_data_size * sizeof(uint32_t),GFP_KERNEL);
			...
			case GOLDEN_SELF_RX_DATA_TYPE:
				...
				golden_rx_self_baseC = (uint32_t *)kzalloc(ini_data_size * sizeof(uint32_t),GFP_KERNEL);
			...
			case GOLDEN_SELF_TX_DATA_TYPE:
				...
				golden_tx_self_baseC = (uint32_t *)kzalloc(ini_data_size * sizeof(uint32_t),GFP_KERNEL);
...
static size_t himax_get_comma_data(char *strStart, size_t searchSize, uint32_t *data, size_t bufSize , int* type)
{
	...
	do{
		...
		if(!commaPos)//data without comma == last data 
		{
			...
			dataLength = itemStart + searchSize -itemOffset - pDataPos + 1;//itemStart + serchSize  -itemOffset point to  ==> " }" ahead one valid data 
			if(pDataPos == hexDataPos)//hex data
			{
				*data = himax_hex_str_to_int(pDataPos+2, 2);//move the point to the string "0x next char location
			}
			else//dec data
			{
		...
		else if(commaPos > pDataPos)//data before comma     as txt data  "0x55,"  hexDataPos -> 0x55, <-commaPos
		{
			if(pDataPos == hexDataPos)//hex data
			{
				...
				{
					if(!(strncmp(itemStart+2,CONFIG_TABLE_FE_REG_MATCH_STR,CONFIG_TABLE_FE_REG_MATCH_SIZE )))//as txt data "0xFE(55),
					{
						*type = 1;
		...
		else//data after comma as txt data  "EN_DEFAULT_CFG, 0x55"   commaPos ->, 0x55<-hexDataPos
		{
			...
			else
			{
				...
				{
					if(!(strncmp(itemStart+2, charMutualSelfRefArray[i].name, strlen(charMutualSelfRefArray[i].name) )))//Compare string form "{ " <= next char
					{
						break;
					}
				} 

				if(i < CHAR_MUTUAL_SELF_REF_NUM)//find char reg in array
				{
...
int8_t himax_load_config_table(void)
{
	...
	if (!IS_ERR(fn))
	{
		...
		do{
			...
			do{
				findStartCharPtr = strnstr(buff_cursor,"{ ",maxItemStrSize -(( buff_cursor - result_buff )+1));// find "{ " locarion

				if (!findStartCharPtr) {
					buff_cursor = result_buff + maxItemStrSize -2;//point to the second end of result_buff      "....{" case 
				...
				findEndCharPtr = strnstr(findStartCharPtr," }",maxItemStrSize - (( buff_cursor - result_buff )+1) - itemStartOffset);//find "{ " locarion
				
				if (!findEndCharPtr) {
					if(itemStartOffset)
					{
						buff_cursor = findStartCharPtr - 1;//point to the "{ " ahead ont byte addr
					}
					I("%s: invalid config table without } \n",__func__);
					break;
				}
				...
				iDataBufLoadSize = himax_get_comma_data(findStartCharPtr, itemEndOffset - itemStartOffset, dataBuf, maxItemDataSize,&type);
				...
				if(type == 1)
				{
					printk("HIMAX [HXTP] wdd:0xFE%02x %02x %02x ... %02x %02x \n", dataBuf[0], dataBuf[1], dataBuf[2], dataBuf[iDataBufLoadSize-1],dataBuf[iDataBufLoadSize]);
				...
				else if(type > 1)
				{
					if(iDataBufLoadSize > 1)
					{
						...
						printk("HIMAX [HXTP] wdd:mutal self data%d %d %d ... %d last data = %d iDataBufLoadSize = %d \n", dataBuf[0], dataBuf[1], dataBuf[2], dataBuf[iDataBufLoadSize-2], dataBuf[iDataBufLoadSize-1], iDataBufLoadSize);
					}
					else
					{
				...
				{
					printk("HIMAX [HXTP] wdd:%02x %02x %02x ... %02x %02x \n", dataBuf[0], dataBuf[1], dataBuf[2], dataBuf[iDataBufLoadSize-1],dataBuf[iDataBufLoadSize]);
					if(dataBuf[0] > 0x20)
						himax_write_config_table(type, dataBuf, iDataBufLoadSize);
	...
	for (loop_i = 0; loop_i < y_channel; loop_i++)
	{
		...
		for (loop_j = 0; loop_j < x_channel; loop_j++)
		{
			if (loop_j != x_channel - 1)
				snprintf(temp, sizeof(temp),"%d,", hx_ito->ito_log_baseC_mul_data[loop_i * x_channel + loop_j]);
			else
...
void himax_ito_mul_log_func(void)
{
	...
	{
		case BANK:
			strcpy(log_type_name, "BANK");
			break;
		case DC:
			strcpy(log_type_name, "DC");
			break;
		case BASEC:
			strcpy(log_type_name, "BASEC");
			break;
		case IIR:
			strcpy(log_type_name, "IIR");
			break;
		default:
			strcpy(log_type_name, "NULL");
	}
...
#SORTING_MODE~Sorting~BaseCPer~Pass~%s~NORMAL~NORMAL~NORMAL~NORMAL~NORMAL\n", log_type_name);
	...
	for (loop_i = 0; loop_i < y_channel; loop_i++)
	{
		...
		{
			if (loop_j != x_channel - 1)
				snprintf(temp, sizeof(temp),"%d,", hx_ito->ito_log_mul_data[loop_i * x_channel + loop_j]);
			else
...
void himax_ito_fail_log_func(uint8_t test_type, int16_t *data, uint16_t test_result)
{
	...
	switch(test_type)
	{
		case MUTUAL_PERCENT_LIMIT:
			strcpy(log_type_name, "MUTUAL_PERCENT_LIMIT");
			break;
		case SELF_PERCENT_LIMIT:
			strcpy(log_type_name, "SELF_PERCENT_LIMIT");
			buf_shift = x_channel;
			break;
		case MUTUAL_DEV_TX:
			strcpy(log_type_name, "MUTUAL_DEV_TX");
			break;
		case MUTUAL_DEV_RX:
			strcpy(log_type_name, "MUTUAL_DEV_RX");
			break;
		case SELF_DEV_TX:
			strcpy(log_type_name, "SELF_DEV_TX");
			break;
		case SELF_DEV_RX:
			strcpy(log_type_name, "SELF_DEV_RX");
			break;
		case MUTUAL_IIR_NOISE:
			strcpy(log_type_name, "MUTUAL_IIR_NOISE");
			break;
		case SELF_IIR_NOISE:
			strcpy(log_type_name, "SELF_IIR_NOISE");
			buf_shift = x_channel;
			break;
		default:
			strcpy(log_type_name, "NULL");
	...
	if(test_type == MUTUAL_PERCENT_LIMIT || test_type == MUTUAL_DEV_TX || test_type == MUTUAL_DEV_RX || test_type == MUTUAL_IIR_NOISE)
	{
		for (loop_i = 0; loop_i < x_channel + 1; loop_i++)
		{
			memset(temp,'\0',sizeof(temp));
...
static uint16_t self_dev_rx_check(void) //(Self_RX_DeviationCheckItem)
{
    ...
    {
        res = ((int32_t)((hx_ito->self_baseC[i] - hx_ito->self_baseC[i + 1]) - (golden_rx_self_baseC[i] - golden_rx_self_baseC[i + 1])) * 100 / (int32_t)golden_rx_self_baseC[i]);
...
static uint16_t self_dev_tx_check(void) //(Self_TX_DeviationCheckItem)
{
    ...
    {
        res = ((int32_t)((hx_ito->self_baseC[x_channel + i] - hx_ito->self_baseC[x_channel + i + 1]) - (golden_tx_self_baseC[i] - golden_tx_self_baseC[i + 1])) * 100 / (int32_t)golden_tx_self_baseC[i]);
	...
	for (i = 0; i < x_channel; i++)
	{
		for (j = 0; j < y_channel; j++)	//Calculate deviation of all block
		{
		    ...
		    if(golden_mutual_baseC[i + j * x_channel ] == 0)
				...
				baseC_avg_except_pre = (rx_channel_sum_arr[i] - hx_ito->mutual_baseC[i + j * x_channel]) / (y_channel_test - 1);
				baseC_avg_except_pre_ref = (rx_channel_sum_ref_arr[i] - golden_mutual_baseC[i + j * x_channel]) / (y_channel_test - 1);
	
				res = (int32_t)((hx_ito->mutual_baseC[i + j * x_channel] - baseC_avg_except_pre) - (golden_mutual_baseC[i + j * x_channel] - baseC_avg_except_pre_ref)) * 100 / (int32_t)golden_mutual_baseC[i + j * x_channel];
				mutual_dev[i + j * x_channel ] = (int16_t)res;

				if( mutual_dev[i + j * x_channel ] < mutual_dev_fail_upper && mutual_dev[i + j * x_channel ] > mutual_dev_fail_lower )
					hx_ito->mutual_rx_dev_result[i + j * x_channel] = mutual_dev[i + j * x_channel]; //Pass
				else
				{
					hx_ito->mutual_rx_dev_result[i + j * x_channel] = mutual_dev[i + j * x_channel]; //Fail
					test_result++;		
				}

#ifdef ITO_DEBUG_DIS
				I("rx baseC_avg_except_pre = %5d, baseC_avg_except_pre_ref = %5d\n", baseC_avg_except_pre, baseC_avg_except_pre_ref);
				I("rx PER = %5d\n", (hx_ito->mutual_baseC[i + j * x_channel] - baseC_avg_except_pre) - (golden_mutual_baseC[i + j * x_channel] - baseC_avg_except_pre_ref));
				I("rx res = %5d\n", res);
				I("mutual_dev[%d][%d] = %5d \n", i, j, mutual_dev[i + j * x_channel]);
				I("mutual_rx_dev_result[%d][%d] = %5d \n", i, j, hx_ito->mutual_rx_dev_result[i + j * x_channel]);
#endif
			}
		}
	}
...
static uint16_t mutual_dev_tx_check(void) //(MutualDeviationTxSideCheck)
{
	...
	for (i = 0; i < y_channel; i++) //Calculate aversge of TX per line
	{
	    ...
	    for (j = 0; j < hx_ito->mkey_num; j++)	//Calculate m_key in last tx
	    {
	      ...
	      res = (int32_t)(hx_ito->mutual_baseC[i * x_channel + location] - golden_mutual_baseC[i * x_channel + location]) * 100 / (int32_t)golden_mutual_baseC[i * x_channel + location];
	    	mutual_dev[i * x_channel + location] = (int16_t)res;
	    	
	    	if( mutual_dev[i * x_channel + location] < mutual_key_dev_fail_upper && mutual_dev[i * x_channel + location] > mutual_key_dev_fail_lower )
	    	{
	    		hx_ito->mutual_tx_dev_result[i * x_channel + location] = mutual_dev[i * x_channel + location]; //Pass
	    	}
				else
				{
					hx_ito->mutual_tx_dev_result[i * x_channel + location] = mutual_dev[i * x_channel + location]; //Fail
					test_result++;
				}
#ifdef ITO_DEBUG_DIS
				I("mutual_dev[%d][%d] = %5d \n", j, i, mutual_dev[i * x_channel + location]);
				I("mutual_tx_dev_result[%d][%d] = %5d \n", j, i, hx_ito->mutual_tx_dev_result[i * x_channel + location]);
#endif
	    }
		}
		else
		{
			for (j = 0; j < x_channel; j++)	//Calculate deviation of all block
			{
				...
				baseC_avg_except_pre = (tx_channel_sum_arr[i] - hx_ito->mutual_baseC[i * x_channel + j]) / (x_channel - 1);
				baseC_avg_except_pre_ref = (tx_channel_sum_ref_arr[i] - golden_mutual_baseC[i * x_channel + j]) / (x_channel - 1);
	
				res = (int32_t)((hx_ito->mutual_baseC[i * x_channel + j] - baseC_avg_except_pre) - (golden_mutual_baseC[i * x_channel + j] - baseC_avg_except_pre_ref)) * 100 / (int32_t)golden_mutual_baseC[i * x_channel + j];
				mutual_dev[i * x_channel + j] = (int16_t)res;
	
				if( mutual_dev[i * x_channel + j] < mutual_dev_fail_upper && mutual_dev[i * x_channel + j] > mutual_dev_fail_lower )
				{
					hx_ito->mutual_tx_dev_result[i * x_channel + j] = mutual_dev[i * x_channel + j]; //Pass
				}
				else
				{
					hx_ito->mutual_tx_dev_result[i * x_channel + j] = mutual_dev[i * x_channel + j]; //Fail
					test_result++;
				}
#ifdef ITO_DEBUG_DIS
				I("baseC_avg_except_pre = %5d, baseC_avg_except_pre_ref = %5d\n", baseC_avg_except_pre, baseC_avg_except_pre_ref);
				I("PER = %5d\n", (hx_ito->mutual_baseC[i * x_channel + j] - baseC_avg_except_pre) - (golden_mutual_baseC[i * x_channel + j] - baseC_avg_except_pre_ref));
				I("res = %5d\n", res);
				I("mutual_dev[%d][%d] = %5d \n", j, i, mutual_dev[i * x_channel + j]);
				I("mutual_tx_dev_result[%d][%d] = %5d \n", j, i, hx_ito->mutual_tx_dev_result[i * x_channel + j]);
#endif
			}
		}
	}
...
static uint16_t baseC_mutual_percent_limit_check(void) //(BaseC_PerLimitCheck)Check BaseC per block 
{
	...
	for (i = 0; i < x_channel; i++)	//Mutual test
	{
	  ...
	  {
	  	res = hx_ito->mutual_baseC[i + j * x_channel]  * 100 / mutual_base_c_max;//fail result : percent data
      hx_ito->mutual_baseC_percent_result[i + j * x_channel] = res;

      if (hx_ito->mutual_baseC[i + j * x_channel] * 10 < (min_mutual_baseC_percent * mutual_base_c_max / 10)
      	|| hx_ito->mutual_baseC[i + j * x_channel] * 10 > (max_mutual_baseC_percent * mutual_base_c_max / 10))
      {
      	test_result++;
      ...
      else
      {
        ...
        //I("mutual_percent[%d][%d] pass \n", i, j);
			...
			if(i < mutual_num)
			{
				I("mutual_baseC[%d][%d] = %5d  mutual_base_c_max = %5d \n", i, j, hx_ito->mutual_baseC[i + j * x_channel], mutual_base_c_max);
				I("mutual_baseC_percent_result[%d][%d] = %5d \n", i, j, hx_ito->mutual_baseC_percent_result[i + j * x_channel]);
			}
			if( i % 8 == 7)
				I("\n");
#endif
...
static void cal_mutual_parameter(void) //(CalculateMutParameter)
{
    ...
    if (VR7_Enable_Mutual == 0)
    {
        ...
        {
            Mut_Sense_Cap_PerBank = (VR1_M_V - VR3_M_V + VR7_V * VR7_Enable_Mutual) * Bank_Cap_pf * MULITPLE_SCALE / (TX_Voltage_P - TX_Voltage_N);
        }
        else
        {
            Mut_Sense_Cap_PerBank = (VR1_M_V - VR3_V + VR7_V * VR7_Enable_Mutual) * Bank_Cap_pf * MULITPLE_SCALE / (TX_Voltage_P - TX_Voltage_N);
        }
    }
    else
    {
        if (Mutual_TwoSetVR && Is2T3R)
        {
            Mut_Sense_Cap_PerBank = (VR1_M_V - VR3_M_V + VR7_V * VR7_Enable_Mutual) * Bank_Cap_pf * 70 / (TX_Voltage_P - TX_Voltage_N); //0.7
        }
        else
        {
            Mut_Sense_Cap_PerBank = (VR1_M_V - VR3_V + VR7_V * VR7_Enable_Mutual) * Bank_Cap_pf * 70 / (TX_Voltage_P - TX_Voltage_N); //0.7
        }
    }
  ...
  mutual_base_c_max = (255 * Mut_Sense_Cap_PerBank + Mutual_BaseLine * Mut_Sense_Cap_PerDAC * Down_Scale / OSR_Mutual) / 10; //MPAP : mutual_bank_cap
...
static void cal_self_rx_parameter(void) //CalculateSelfRXParameter
{
	...
	{
	    SlfRX_Sense_Cap_PerBank = (VR1_S_V - VR3_V + VR7_V * VR7_Enable_Self) * Bank_Cap_pf * 100 / (VR3_V - VR2_S_V);
	}
	else
	{
	    SlfRX_Sense_Cap_PerBank = (VR1_S_V - VR3_V + VR7_V * VR7_Enable_Self) * Bank_Cap_pf * 70 / (VR3_V - VR2_S_V); //0.7
	}
	SlfRX_Sense_Cap_PerDAC = (VR4_S_V - VR3_V) * DAC_Ratio * DAC_Cap_pf / (VR3_V - VR2_S_V) ;

	self_rx_base_c_max =  (SlfRX_Sense_Cap_PerBank * 255  + Self_BaseLineUp * SlfRX_Sense_Cap_PerDAC * Down_Scale / OSR_SelfRX) / 10;
...
static void cal_self_tx_parameter(void) //CalculateSelfTXParameter
{
    ...
    if (VR7_Enable_Self == 0)
    {
        SlfTX_Sense_Cap_PerBank = (VR1_S_V - VR3_V + VR7_V * VR7_Enable_Self) * Bank_Cap_pf * 100 / (VR3_V - VR2_S_V);
    }
    else
    {
        SlfTX_Sense_Cap_PerBank = (VR1_S_V - VR3_V + VR7_V * VR7_Enable_Self) * Bank_Cap_pf * 70/ (VR3_V - VR2_S_V);
    }
    SlfTX_Sense_Cap_PerDAC = (VR4_S_V - VR3_V) * DAC_Ratio * DAC_Cap_pf / (VR3_V - VR2_S_V);

	self_tx_base_c_max = (SlfTX_Sense_Cap_PerBank * 255  + Self_BaseLineUp * SlfTX_Sense_Cap_PerDAC * Down_Scale / OSR_SelfTX) / 10;
...
static void cal_vr_parameter(bool is_mutual, int tx_num, int rx_num) //(CalculateVRParameter)
{
	...
	if (Mutual_TwoSetVR)
  {
      ...
      CycleNumber = (temp_data[0] & 0xF0) >> 4;//MappingObj.GetCycleNumber(rx_num); //Get Cycle number : 1~3 //FE(73) High byte
...
static void get_self_baseC(void) //(GetSelfRxBaseC + GetSelfTxBaseC)
{
	...
	for (i = 0; i < x_channel + y_channel; i++)
	{
      ...
      temp_double = hx_ito->self_bank_data[i] * SlfRX_Sense_Cap_PerBank + hx_ito->self_dc_data[i] * Down_Scale * SlfRX_Sense_Cap_PerDAC / OSR_SelfRX;
    ...
    else	//TX
    {
    	...
    	//original scaling 10000
      temp_double = hx_ito->self_bank_data[i] * SlfTX_Sense_Cap_PerBank + hx_ito->self_dc_data[i] * Down_Scale * SlfTX_Sense_Cap_PerDAC / OSR_SelfTX;
      hx_ito->self_baseC[i] = temp_double / 10;
      I("self_baseC[%d] = %d\t", i, temp_double / 10);
    }
...
#ifdef HIMAX_ITO_TEST_LOG
	memcpy(hx_ito->ito_log_baseC_self_data, hx_ito->self_baseC, self_num * sizeof(uint32_t)); //Self BaseC data
...
static void get_mutual_baseC(void) //(GetMutualBaseC)
{
    ...
    for (j = 0; j < x_channel; j++)
    {
      ...
      cal_mutual_parameter();
			...
			//original scaling 10000
      temp_double = hx_ito->mutual_bank_data[i * x_channel + j] * Mut_Sense_Cap_PerBank + hx_ito->mutual_dc_data[i * x_channel + j] * Down_Scale * Mut_Sense_Cap_PerDAC / OSR_Mutual;
      hx_ito->mutual_baseC[i * x_channel + j] = temp_double / 10;
      //hx_ito->mutual_baseC[i][j] = temp_double / 10;
    }
	}
#ifdef HIMAX_ITO_TEST_LOG
	memcpy(hx_ito->ito_log_baseC_mul_data, hx_ito->mutual_baseC, mutual_num * sizeof(uint32_t)); //Mutual BaseC data
...
/***** OSR - 0xE1 *****/
	...
	I("%s : ChrageDischarge_Mutual = %d, ChrageDischarge_SelfRX = %d, ChrageDischarge_SelfTX = %d, ChrageDischarge_SelfBT = %d, \n", 
	__func__, ChrageDischarge_Mutual, ChrageDischarge_SelfRX, ChrageDischarge_SelfTX, ChrageDischarge_SelfBT);
	I("%s : OSR_Mutual = %d, OSR_SelfRX = %d, OSR_SelfTX = %d, OSR_SelfBT = %d, \n", __func__, OSR_Mutual, OSR_SelfRX, OSR_SelfTX, OSR_SelfBT);
/***** MUL_8_16 - 0x3F *****/
	MUL_8_16_Enable_Mutual = c6[1] & 0x02;
	MUL_8_16_Enable_Self = c6[1] & 0x02;

	I("%s : MUL_8_16_Enable_Mutual = %d, MUL_8_16_Enable_Self = %d \n", __func__, MUL_8_16_Enable_Mutual, MUL_8_16_Enable_Self);
/***** P_PULSE - 0xBC *****/
...
/***** Check Auto Bank Cut and Two Set VR - 0xFE(02) *****/
	...
	I("%s : TwoSet_VR1_M_1 = %d, TwoSet_VR3_M_1 = %d, TwoSet_VR4_M_1 = %d, TwoSet_VR1_M_2 = %d, \n", __func__, TwoSet_VR1_M_1, TwoSet_VR3_M_1, TwoSet_VR4_M_1, TwoSet_VR1_M_2);
	I("%s : TwoSet_VR3_M_2 = %d, TwoSet_VR4_M_2 = %d, TwoSet_VR1_S_RX = %d, TwoSet_VR3_S_RX = %d, \n", __func__, TwoSet_VR3_M_2, TwoSet_VR4_M_2, TwoSet_VR1_S_RX, TwoSet_VR3_S_RX);
	I("%s : TwoSet_VR4_S_RX = %d, TwoSet_VR1_S_TX = %d, TwoSet_VR3_S_TX = %d, TwoSet_VR4_S_TX = %d, \n", __func__, TwoSet_VR4_S_RX, TwoSet_VR1_S_TX, TwoSet_VR3_S_TX, TwoSet_VR4_S_TX);
}


static void himax_get_test_raw_data(uint8_t diag_command, uint16_t mutual_num, uint16_t self_num, uint16_t *mutual_data, uint16_t *self_data)
{
#ifdef HIMAX_ITO_TEST_LOG
#ifdef ITO_DEBUG_DIS
	int i = 0;
	...
	if(diag_command == 3)
	{
		...
		memcpy(hx_ito->ito_log_mul_data, hx_ito->mutual_bank_data, mutual_num * sizeof(uint16_t)); //Mutual BANK data
		memcpy(hx_ito->ito_log_self_data, hx_ito->self_bank_data, self_num * sizeof(uint16_t));	//Self BANK data
		...
		for(i = 0;i < mutual_num + self_num ;i++)
		{
			if(i < mutual_num)
				...
				I("ito_log_mul_data self bank[%d] = %3d \t", i - mutual_num, hx_ito->ito_log_mul_data[i - mutual_num]);
			if( i%8 == 7)
				I("\n");
		}
#endif
	...
	if(diag_command == 2)
	{
...
#ifdef HIMAX_ITO_TEST_LOG
		// run store data to sdcard
		memcpy(hx_ito->ito_log_mul_data, hx_ito->mutual_dc_data, mutual_num * sizeof(uint16_t)); //Mutual DC data
		memcpy(hx_ito->ito_log_self_data, hx_ito->self_dc_data, self_num * sizeof(uint16_t));	//Self DC data
		...
		for(i = 0;i < mutual_num + self_num ;i++)
		{
			if(i < mutual_num)
				...
				I("ito_log_mul_data self DC[%d] = %3d \t", i - mutual_num, hx_ito->ito_log_mul_data[i - mutual_num]);
			if( i%8 == 7)
				I("\n");
		}
#endif
	...
	if(diag_command == 1)
	{
...
#ifdef HIMAX_ITO_TEST_LOG
		// run store data to sdcard
		memcpy(hx_ito->ito_log_mul_data, hx_ito->mutual_iir_data, mutual_num * sizeof(uint16_t)); //Mutual DC data
		memcpy(hx_ito->ito_log_self_data, hx_ito->self_iir_data, self_num * sizeof(uint16_t));	//Self DC data
		...
		for(i = 0;i < mutual_num + self_num ;i++)
		{
			if(i < mutual_num)
				...
				I("ito_log_mul_data self IIR[%d] = %3d \t", i - mutual_num, hx_ito->ito_log_mul_data[i - mutual_num]);
			if( i%8 == 7)
				I("\n");
		}
#endif
