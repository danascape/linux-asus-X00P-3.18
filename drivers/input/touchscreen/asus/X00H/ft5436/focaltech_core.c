...
static struct task_struct *thread_tpd;
    ...
    do {
		...
		if (ret < 0) {
			...
			if(uc_tp_chip_id == chip_types.chip_idh) {
				...
				FTS_INFO("Not focaltech IC,probe fail");
				return -1;
			}
		} else if (reg_value == chip_types.chip_idh) {
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    err = fts_ctpm_get_upgrade_array();
	if(err < 0){
		input_free_device(input_dev);
		return err; 
	}
    ...
    if(err < 0) {
        FTS_ERROR("FTS is no valid\n");
        goto free_gpio;
    }

	thread_tpd = kthread_run(touch_event_handler, 0, FTS_DRIVER_NAME);
	if (IS_ERR(thread_tpd)) {
		err = PTR_ERR(thread_tpd);
		printk("[TPD]Failed to create kernel thread_tpd,ret=%d!",
			  err);
	}
