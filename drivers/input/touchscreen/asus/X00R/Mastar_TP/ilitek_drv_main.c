...
u8 g_GestureEn = 0;  //mstar added for GestureWakeup 20180718 
...
u32 g_GestureWakeupMode[2] = {0x00000000, 0x00000000};//mstar added for GestureWakeup 20180718
...
//mstar 20180718 begin >> [2/8]add for GESTURE_WAKEUP
static struct proc_dir_entry *_gProcAndroidTouchEntry = NULL;
static struct proc_dir_entry *_gProcAndroidTouchGestureEnFile = NULL;
static struct proc_dir_entry *_gProcAndroidTouchGestureModeFile = NULL;
//mstar 20180718 end >> [2/8]add for GESTURE_WAKEUP
...
//mstar 20180718 begin >> [3/8]add for GESTURE_WAKEUP
static ssize_t _gProcAndroidTouchGestureEnRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos);
static ssize_t _gProcAndroidTouchGestureEnWrite(struct file *file, const char *buff,
                                size_t len, loff_t *pos);
static ssize_t _gProcAndroidTouchGestureModeRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos);
static ssize_t _gProcAndroidTouchGestureModeWrite(struct file *file, const char *buff,
                                size_t len, loff_t *pos);
//mstar 20180718 end >> [3/8]add for GESTURE_WAKEUP
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
//mstar 20180718 begin >> [4/8]add for GESTURE_WAKEUP
static const struct file_operations _gProcAndroidTouchGestureEn = { 
    .read = _gProcAndroidTouchGestureEnRead,
    .write = _gProcAndroidTouchGestureEnWrite,
};
static const struct file_operations _gProcAndroidTouchGestureMode = { 
    .read = _gProcAndroidTouchGestureModeRead,
    .write = _gProcAndroidTouchGestureModeWrite,
};
//mstar 20180718 end >> [4/8]add for GESTURE_WAKEUP
...
void DrvCheckFWSupportDriver(void)
{
    ...
    mdelay(I2C_SMBUS_READ_COMMAND_DELAY_FOR_PLATFORM);
    IicReadData(SLAVE_I2C_ID_DWI2C, &szRxData[0], 4); 
    sscanf(DEVICE_DRIVER_RELEASE_VERSION,"%x.%x.%x.%x", (u32*)&szDriverVer[0], (u32*)&szDriverVer[1], (u32*)&szDriverVer[2], (u32*)&szDriverVer[3]);
...
static ssize_t DrvMainProcfsMpTestCustomisedRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	...
	{
		DBG(&g_I2cClient->dev,"copy_to_user error \n");
	return -EFAULT;
...
//mstar 20180718 begin >> [5/8]add for GESTURE_WAKEUP
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
static ssize_t _gProcAndroidTouchGestureEnRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    u32 nLength = 0;
    ...
    DBG(&g_I2cClient->dev, "g_GestureEn = 0x%x\n", g_GestureEn);

    nLength = sprintf(nUserTempBuffer, "%x", g_GestureEn);
    if(copy_to_user(pBuffer, nUserTempBuffer, nLength))
	{
		DBG(&g_I2cClient->dev,"copy to user error\n");
        return -EFAULT;
                                ...
                                size_t len, loff_t *pos)
{
    ...
    if (len >= 80)
    {
		DBG(&g_I2cClient->dev,"_gProcAndroidTouchGestureEnWrite fail\n");
        return -EFAULT;
    }
	...
	DBG(&g_I2cClient->dev,"g_GestureEn= %d \n", g_GestureEn);
    }
    else if (buf[0] == '1')
    {
                                   ...
                                   size_t len, loff_t *pos)
{
    ...
    if (len >= 80)
    {
		DBG(&g_I2cClient->dev, "Fail _gProcAndroidTouchGestureModeWrite");
        return -EFAULT;
    }
    if (copy_from_user(buf, buff, len))
    {
	...
	//DBG(&g_I2cClient->dev, "buf1[%d] = %d\n",i, buf1[i]);
	wind_log("buf1[%d] = %d\n",i, buf1[i]);
	}

	

    drvbuf[0]  = buf1[0];  //double
    drvbuf[1]  = buf1[1];  //up
    drvbuf[6] = buf1[2];   //w
    drvbuf[11]  = buf1[3]; //s
    drvbuf[8] = buf1[4];  //e
    ...
    for (i=0; i<16; i++)
    {
       g_GestureWakeupMode[0] = g_GestureWakeupMode[0] | (drvbuf[i]<<i);
	   
   	//DBG(&g_I2cClient->dev, "drvbuf %d = %d\n",i, drvbuf[i]);
    }
    //DBG(&g_I2cClient->dev, "g_GestureWakeupMode = 0x%x\n", g_GestureWakeupMode[0]);
...
//mstar 20180718 end >> [5/8]add for GESTURE_WAKEUP
...
#define CONFIG_ENABLE_TP_LOG
#ifdef CONFIG_ENABLE_TP_LOG
#define TP_LOG "TpLog"
#define TP_LOG_DEBUG "debug"

#define  MAX_TP_LOG_NUM (10)
#define  MAX_TP_LOG_LEN (128)

typedef struct TpLog
{
    char **queue;
    int front;
    int rear;

     void (*construct)(struct TpLog*);//used only by newTpLog
      void (*destructor)(struct TpLog*);//public 

    void (*in)(struct TpLog*,char*);//private
    int (*out)(struct TpLog*,char*);//public
    int (*set)(struct TpLog*,char*,int);//public

  	struct proc_dir_entry * tpLogEntry;
	struct proc_dir_entry * tpLogDebugEntry;

	struct mutex logMutex;

}TpLog;

TpLog* newTpLog(void);
static void constructTpLog(TpLog* this);

static void inTpLog(TpLog* this,char* logIn)
{
    if((this->rear+1)%MAX_TP_LOG_NUM==this->front)
    {
	    char tmpBuf[MAX_TP_LOG_LEN]={0};
	    this->out(this,tmpBuf);
    }
    
    memset( this->queue[this->rear],0,MAX_TP_LOG_LEN);
    sprintf( this->queue[this->rear],"%s",logIn);

    printk("TpLog: in %s ; logNum = %d\n",this->queue[this->rear],(int)((this->rear-this->front+MAX_TP_LOG_NUM)%MAX_TP_LOG_NUM+1));
    this->rear = (this->rear+1)%MAX_TP_LOG_NUM;
}

static int outTpLog(TpLog* this,char* logOut)
{
    int len = 0;
    mutex_lock(&(this->logMutex));

    if(this->rear==this->front)
    {
        printk("TpLog: out empty\n");
	 len = 0;
    }
    else
    {
	 len = sprintf(logOut,"%s\n",this->queue[this->front]);
    	 printk("TpLog: out %s ; ",this->queue[this->front]);
        this->front = (this->front+1)%MAX_TP_LOG_NUM;
        printk("logNum = %d ; len=%d\n",(int)((this->rear-this->front+MAX_TP_LOG_NUM)%MAX_TP_LOG_NUM),len);
    }
	
    mutex_unlock(&(this->logMutex));
		
    return len;
}

static int setTpLog(TpLog* this,char* pPacket,int nLength)
{
	static char log[MAX_TP_LOG_LEN] = {0};
	static int cnt = 0;
	int i = 0;
    if (pPacket[0]>>4 == 0x2)
   {
		...
		{
			log[i+cnt*(nLength-1)] = pPacket[1+i];
		}

		if((pPacket[0]&0x000F)==1)
		{
			printk("TpLog: set: %s\n",log);
			this->in(this,log);
			memset(log,0,MAX_TP_LOG_LEN);
			cnt=0;
		}
		else
		{
    ...
    {
        printk("TpLog: set null\n");
        return 0;//set null
    }

}

static ssize_t tpLogDebugRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    int len = 0;
    char nUserTempBuffer[MAX_TP_LOG_LEN] = {0}; 
    TpLog * this = newTpLog();
    ...
    memset(nUserTempBuffer,0,MAX_TP_LOG_LEN);

   len = this->out(this,nUserTempBuffer);
	
    printk("TpLog: readDebug: %s;len=%d\n",nUserTempBuffer,len);
    if(copy_to_user(pBuffer, nUserTempBuffer, len))
	{
        return -EFAULT;
	}	
    *pPos += len;
...
static const struct file_operations tpLogDebug = { 
    .read = tpLogDebugRead,
};

static void destructorTpLog(TpLog* this)
{
	if(this->construct == constructTpLog)
	{
		int i = 0;
		for(i=0;i<MAX_TP_LOG_NUM;i++)
		{
			kfree(this->queue[i]);
		}
		kfree(this->queue);
		...
		remove_proc_entry(TP_LOG_DEBUG, this->tpLogEntry);
                remove_proc_entry(TP_LOG, NULL);

		this->construct = NULL;

		printk("TpLog: destructor\n");
	}
}
...
{
        int i = 0;
        this->queue = (char**)kmalloc(sizeof(char*)*MAX_TP_LOG_NUM, GFP_KERNEL);
        for(i=0;i<MAX_TP_LOG_NUM;i++)
        {
            this->queue[i]=(char*)kmalloc(sizeof(char)*MAX_TP_LOG_LEN, GFP_KERNEL);
        }
        this->front = 0;
        this->rear = 0;
	 this->destructor = destructorTpLog;
        this->in = inTpLog;
        this->out = outTpLog;
        this->set = setTpLog;

	this->tpLogEntry = proc_mkdir(TP_LOG, NULL);
	this->tpLogDebugEntry= proc_create(TP_LOG_DEBUG, 0666, this->tpLogEntry , &tpLogDebug);
	
	mutex_init(&(this->logMutex));

	printk("TpLog: construct\n");
...
//mstar 20180718 begin >> [6/8]add for GESTURE_WAKEUP

	if (_gProcAndroidTouchGestureEnFile != NULL)
    {
        remove_proc_entry(PROC_ANDROID_TOUCH_GESTURE_EN, _gProcAndroidTouchEntry);  
        _gProcAndroidTouchGestureEnFile = NULL;  		
        DBG(&g_I2cClient->dev, "Remove procfs file node(%s) OK!\n", PROC_ANDROID_TOUCH_GESTURE_EN);
    }

	if (_gProcAndroidTouchGestureModeFile != NULL)
    {
        remove_proc_entry(PROC_ANDROID_TOUCH_GESTURE_MODE, _gProcAndroidTouchEntry);  
        _gProcAndroidTouchGestureModeFile = NULL;  		
        DBG(&g_I2cClient->dev, "Remove procfs file node(%s) OK!\n", PROC_ANDROID_TOUCH_GESTURE_MODE);
    }

    if (_gProcAndroidTouchEntry != NULL)
    {
        remove_proc_entry(PROC_ANDROID_TOUCH, NULL);  
        _gProcAndroidTouchEntry = NULL;  		
        DBG(&g_I2cClient->dev, "Remove procfs file node(%s) OK!\n", PROC_ANDROID_TOUCH);
    }
	
//mstar 20180718 end >> [6/8]add for GESTURE_WAKEUP
...
#endif //CONFIG_TP_HAVE_KEY
*/
//zhaopengfei@wind-mobi.com 20180719 begin >> added for Gesture
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
	...
	set_bit(KEY_CUST_01, g_InputDevice->keybit);
    set_bit(KEY_CUST_02, g_InputDevice->keybit);
    set_bit(KEY_CUST_03, g_InputDevice->keybit);
    set_bit(KEY_CUST_04, g_InputDevice->keybit);
    set_bit(KEY_CUST_05, g_InputDevice->keybit);
    set_bit(KEY_CUST_06, g_InputDevice->keybit);
    set_bit(KEY_CUST_07, g_InputDevice->keybit);
    set_bit(KEY_CUST_08, g_InputDevice->keybit);
    set_bit(KEY_CUST_09, g_InputDevice->keybit);
    set_bit(KEY_CUST_10, g_InputDevice->keybit);
    set_bit(KEY_CUST_11, g_InputDevice->keybit);
    set_bit(KEY_CUST_12, g_InputDevice->keybit);
    set_bit(KEY_CUST_13, g_InputDevice->keybit);
    set_bit(KEY_CUST_14, g_InputDevice->keybit);
    set_bit(KEY_CUST_15, g_InputDevice->keybit);
#endif //CONFIG_ENABLE_GESTURE_WAKEUP
//zhaopengfei@wind-mobi.com 20180719 end >> added for Gesture
...
static s32 _DrvMutualParsePacket(u8 *pPacket, u16 nLength, MutualTouchInfo_t *pInfo) // for MSG28xx/MSG58xxA
{
	...
	{
		TpLog* this = newTpLog();
		if(this->set(this,pPacket,nLength))
		{
			return -1;
		}
	}
    ...
    if (g_GestureWakeupFlag == 1)
    {
        ...
        {
            DBG(&g_I2cClient->dev, "nWakeupMode = 0x%x\n", nWakeupMode);
//mstar 20180718 begin >> [9/8]add for GESTURE_WAKEUP	
            switch (nWakeupMode)
            {
                case 0x58:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_DOUBLE_CLICK_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by DOUBLE_CLICK gesture wakeup.\n");
					wind_log("DOUBLE_CLICK gesture wakeup");
                    input_report_key(g_InputDevice, KEY_CUST_01, 1);
                    input_sync(g_InputDevice);
                    input_report_key(g_InputDevice, KEY_CUST_01, 0);
                    input_sync(g_InputDevice);
                    break;		
                case 0x60:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_UP_DIRECT_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by UP_DIRECT gesture wakeup.\n");
					wind_log("UP_DIRECT gesture wakeup");
//                    input_report_key(g_InputDevice, KEY_UP, 1);
                    input_report_key(g_InputDevice, KEY_CUST_02, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_UP, 0);
                    input_report_key(g_InputDevice, KEY_CUST_02, 0);
                    input_sync(g_InputDevice);
                    break;		
                case 0x61:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_DOWN_DIRECT_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by DOWN_DIRECT gesture wakeup.\n");
					wind_log("DOWN_DIRECT gesture wakeup.");
//                    input_report_key(g_InputDevice, KEY_DOWN, 1);
                    input_report_key(g_InputDevice, KEY_CUST_03, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_DOWN, 0);
                    input_report_key(g_InputDevice, KEY_CUST_03, 0);
                    input_sync(g_InputDevice);
                    break;		
                case 0x62:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_LEFT_DIRECT_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by LEFT_DIRECT gesture wakeup.\n");
					wind_log("LEFT_DIRECT gesture wakeup");
//                  input_report_key(g_InputDevice, KEY_LEFT, 1);
                    input_report_key(g_InputDevice, KEY_CUST_04, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_LEFT, 0);
                    input_report_key(g_InputDevice, KEY_CUST_04, 0);
                    input_sync(g_InputDevice);
                    break;		
                case 0x63:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_RIGHT_DIRECT_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by RIGHT_DIRECT gesture wakeup.\n");
					wind_log("RIGHT_DIRECT gesture wakeup");
//                    input_report_key(g_InputDevice, KEY_RIGHT, 1);
                    input_report_key(g_InputDevice, KEY_CUST_05, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_RIGHT, 0);
                    input_report_key(g_InputDevice, KEY_CUST_05, 0);
                    input_sync(g_InputDevice);
                    break;		
                case 0x64:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_m_CHARACTER_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by m_CHARACTER gesture wakeup.\n");
					wind_log("M_CHARACTER gesture wakeup");
//                    input_report_key(g_InputDevice, KEY_M, 1);
                    input_report_key(g_InputDevice, KEY_CUST_06, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_M, 0);
                    input_report_key(g_InputDevice, KEY_CUST_06, 0);
                    input_sync(g_InputDevice);
                    break;		
                case 0x65:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_W_CHARACTER_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by W_CHARACTER gesture wakeup.\n");
					wind_log("W_CHARACTER gesture wakeup");
//                    input_report_key(g_InputDevice, KEY_W, 1);
                    input_report_key(g_InputDevice, KEY_CUST_07, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_W, 0);
                    input_report_key(g_InputDevice, KEY_CUST_07, 0);
                    input_sync(g_InputDevice);
                    break;		
                case 0x66:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_C_CHARACTER_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by C_CHARACTER gesture wakeup.\n");
					wind_log("C_CHARACTER gesture wakeup");
//                    input_report_key(g_InputDevice, KEY_C, 1);
                    input_report_key(g_InputDevice, KEY_CUST_08, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_C, 0);
                    input_report_key(g_InputDevice, KEY_CUST_08, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x67:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_e_CHARACTER_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by e_CHARACTER gesture wakeup.\n");
					wind_log("e_CHARACTER gesture wakeup");
//                    input_report_key(g_InputDevice, KEY_E, 1);
                    input_report_key(g_InputDevice, KEY_CUST_09, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_E, 0);
                    input_report_key(g_InputDevice, KEY_CUST_09, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x68:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_V_CHARACTER_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by V_CHARACTER gesture wakeup.\n");
					wind_log("V_CHARACTER gesture wakeup");
//                    input_report_key(g_InputDevice, KEY_V, 1);
                    input_report_key(g_InputDevice, KEY_CUST_10, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_V, 0);
                    input_report_key(g_InputDevice, KEY_CUST_10, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x69:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_O_CHARACTER_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by O_CHARACTER gesture wakeup.\n");
					wind_log("O_CHARACTER gesture wakeup");
//                    input_report_key(g_InputDevice, KEY_O, 1);
                    input_report_key(g_InputDevice, KEY_CUST_11, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_O, 0);
                    input_report_key(g_InputDevice, KEY_CUST_11, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x6A:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_S_CHARACTER_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by S_CHARACTER gesture wakeup.\n");
					wind_log("S_CHARACTER gesture wakeup");
//                    input_report_key(g_InputDevice, KEY_S, 1);
                    input_report_key(g_InputDevice, KEY_CUST_12, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_S, 0);
                    input_report_key(g_InputDevice, KEY_CUST_12, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x6B:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_Z_CHARACTER_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by Z_CHARACTER gesture wakeup.\n");
					wind_log("Z_CHARACTER gesture wakeup");
//                    input_report_key(g_InputDevice, KEY_Z, 1);
                    input_report_key(g_InputDevice, KEY_CUST_13, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_Z, 0);
                    input_report_key(g_InputDevice, KEY_CUST_13, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x6C:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_RESERVE1_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by GESTURE_WAKEUP_MODE_RESERVE1_FLAG gesture wakeup.\n");
					wind_log("GESTURE_WAKEUP_MODE_RESERVE1");
//                    input_report_key(g_InputDevice, RESERVER1, 1);
                    input_report_key(g_InputDevice, KEY_CUST_14, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, RESERVER1, 0);
                    input_report_key(g_InputDevice, KEY_CUST_14, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x6D:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_RESERVE2_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by GESTURE_WAKEUP_MODE_RESERVE2_FLAG gesture wakeup.\n");
					wind_log("GESTURE_WAKEUP_MODE_RESERVE2");
//                    input_report_key(g_InputDevice, RESERVER2, 1);
                    input_report_key(g_InputDevice, KEY_CUST_15, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, RESERVER2, 0);
                    input_report_key(g_InputDevice, KEY_CUST_15, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x6E:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_RESERVE3_FLAG;
                    ...
                    //DBG(&g_I2cClient->dev, "Light up screen by GESTURE_WAKEUP_MODE_RESERVE3_FLAG gesture wakeup.\n");
					wind_log("GESTURE_WAKEUP_MODE_RESERVE3");
//                    input_report_key(g_InputDevice, RESERVER3, 1);
                    input_report_key(g_InputDevice, KEY_CUST_15, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, RESERVER3, 0);
                    ...
                    input_sync(g_InputDevice);
                    break;
//mstar 20180718 end >> [9/8]add for GESTURE_WAKEUP	
...
//mstar 20180718 begin >> [7/8]add for GESTURE_WAKEUP	
	_gProcAndroidTouchEntry = proc_mkdir( PROC_ANDROID_TOUCH, NULL);
	if (_gProcAndroidTouchEntry == NULL)
	{
        DBG(&g_I2cClient->dev, "Failed to create _gProcAndroidTouchEntry(%s)!\n", PROC_ANDROID_TOUCH);
		return -ENOMEM;
	}
	
	_gProcAndroidTouchGestureEnFile = proc_create(PROC_ANDROID_TOUCH_GESTURE_EN, PROCFS_AUTHORITY,
									   _gProcAndroidTouchEntry, &_gProcAndroidTouchGestureEn);
	if(_gProcAndroidTouchGestureEnFile == NULL)
	{
        DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n", PROC_ANDROID_TOUCH_GESTURE_EN);
	}
	
	_gProcAndroidTouchGestureModeFile = proc_create(PROC_ANDROID_TOUCH_GESTURE_MODE, PROCFS_AUTHORITY,
										  _gProcAndroidTouchEntry, &_gProcAndroidTouchGestureMode);
	if(_gProcAndroidTouchGestureModeFile == NULL)
	{
        DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n", PROC_ANDROID_TOUCH_GESTURE_MODE);
	}
//mstar 20180718 end >> [7/8]add for GESTURE_WAKEUP
...
#endif //CONFIG_ENABLE_COUNT_REPORT_RATE
//zhaopengfei@wind-mobi.com 20180720 begin 
    _gProcGloveModeEntry = proc_create(PROC_ANDROID_TOUCH_GLOVE_MODE, PROCFS_AUTHORITY, _gProcAndroidTouchEntry, &_gProcGloveMode);	////zhaopengfei@wind-mobi.com add for GLOVE mode 20180719
...
//zhaopengfei@wind-mobi.com 20180720 end 

    _gProcGloveModeEntry = proc_create(PROC_NODE_GLOVE_MODE, PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcGloveMode);	////zhaopengfei@wind-mobi.com add for GLOVE mode 20180719
    if (NULL == _gProcGloveModeEntry)
    {
        DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n", PROC_NODE_GLOVE_MODE);
    }
...
//#endif //CONFIG_ENABLE_GESTURE_WAKEUP
        ...
        else if (*pBlank == FB_BLANK_POWERDOWN && nEvent == FB_EVENT_BLANK)
        {
            ...
            {
                DBG(&g_I2cClient->dev, " g_GestureState = %d, g_ChipType = 0x%X\n", g_GestureState, g_ChipType);
                if((g_GestureEn ==1 )&&(g_GestureWakeupMode[0] != 0x00000000 || g_GestureWakeupMode[1] != 0x00000000)) //mstar added for GestureWakeup 20180718 
                {
...
// zhaopengfei@wind-mobi.com 20180711 end	<<< [2/5] add the mastar device info
	...
	tp_is_probe = 1;	//zhaopengfei@wind-mobi.com 20180810 added   <<< [3/3] add tp probe flag
	wind_log("done, tp_is_probe = %d", tp_is_probe);//zhaopengfei@wind-mobi.com 20180810 added   <<< [3/3] add tp probe flag
    return nRetVal;
}
