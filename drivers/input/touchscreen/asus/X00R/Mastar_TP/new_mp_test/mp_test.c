...
static int new_get_raw_data(s16 *pRawData, u16 count)
{
    ...
    //RegSetLByteValue(0x0F50, 0x00); //i2c access DQMEM
    ...
    //RegSetLByteValue(0x0F50, 0x02); //i2c stop to access DQMEM 
...
static int new_flow_start_test(void)
{
    ...
    if(ret == 0)
    {
        ...
        mp_info("*** New Judge return value = %d ***\n", ret);
    }
