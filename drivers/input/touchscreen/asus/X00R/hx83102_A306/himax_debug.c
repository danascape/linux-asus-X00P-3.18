...
extern int hx83102_input_register(struct hx_83102_ts_data *ts);
#ifdef HX_CHIP_STATUS_MONITOR
extern struct chip_monitor_data *g_hx83102_chip_monitor_data;
#endif
...
int			hx83102_RX_NUM_2					= 0;
int 		hx83102_TX_NUM_2					= 0;
#endif
static uint8_t g_diag_arr_num = 0;
#endif

static int g_max_mutual = 0;
static int g_min_mutual = 255;
static int g_max_self = 0;
static int g_min_self = 255;

#if defined(HX_TP_PROC_SELF_TEST) || defined(CONFIG_TOUCHSCREEN_HIMAX_ITO_TEST)
int g_hx83102_self_test_entered = 0;
#endif

static struct timespec timeStart, timeEnd, timeDelta;
static int g_switch_mode = 0;
extern void hx83102_idle_mode(struct i2c_client *client,int disable);
extern int hx83102_switch_mode(struct i2c_client *client,int mode);
extern void hx83102_return_event_stack(struct i2c_client *client);

#ifdef HX_ZERO_FLASH
extern void hx83102_0f_operation(struct work_struct *work);
extern void hx83102_0f_operation_check(void);
extern void hx83102_sys_reset(void);
#endif
                                   ...
                                   size_t len, loff_t *pos)
{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
        ...
        msleep(20);
        result = hx83102_calculateChecksum(hx83102_ts->client, false);
                                 ...
                                 size_t len, loff_t *pos)
{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
        ...
        if(hx83102_ic_data->vendor_cid_maj_ver < 0 && hx83102_ic_data->vendor_cid_min_ver < 0)
        ...
        if(hx83102_ic_data->vendor_panel_ver < 0)
...
{
    ssize_t ret = 0;
    struct hx_83102_ts_data *ts_data;
    char *temp_buf;
    ...
    if(!HX_PROC_SEND_FLAG)
    {
        ...
        ret += snprintf(temp_buf + ret, len - ret, "attn = %x\n", hx83102_int_gpio_read(ts_data->pdata->gpio_irq));
        if(copy_to_user(buf, temp_buf, len))
            I("%s,here:%d\n",__func__,__LINE__);
        kfree(temp_buf);
        HX_PROC_SEND_FLAG=1;
                                  ...
                                  size_t len, loff_t *pos)
{
    ...
    if (value)
    {
        ret = hx83102_int_en_set(ts->client);
        if (ret == 0)
        {
            ts->irq_enabled = 1;
            hx83102_irq_enable_count = 1;
        }
    }
    else
    {
                                        ...
                                        size_t len, loff_t *pos)
{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
        ...
        hx83102_register_read(hx83102_ts->client, register_command, 128, data, hx83102_cfg_flag);
        ...
        size_t len, loff_t *pos)
{
    ...
    if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' && buf[2] == 'x')
    {
        ...
        if (buf[0] == 'r')
        {
            ...
            {
                length = length - base;
                hx83102_cfg_flag = true;
                memcpy(buf_tmp, data_str + base +1, length);
            }
            else
            {
                hx83102_cfg_flag = false;
        ...
        else if (buf[0] == 'w')
        {
            ...
            if(count < 3)
            {
                ...
                hx83102_register_write(hx83102_ts->client, register_command, byte_length, w_data, hx83102_cfg_flag);
            }
            else
            {
                ...
                {
                    memset(buf_tmp, 0x0, sizeof(buf_tmp));
                    if(hx83102_cfg_flag != 0 && loop_i != 0)
                        byte_length = 2;
                    else
                ...
                byte_length = count - 1;
                hx83102_register_write(hx83102_ts->client, register_command, byte_length, &w_data[0], hx83102_cfg_flag);
            }
        }
        else
            return len;
...
#ifdef HX_TP_PROC_DIAG
int hx83102_set_diag_cmd(struct hx_83102_ic_data *hx83102_ic_data,struct hx_83102_report_data *hx83102_touch_data)
{
	int32_t *mutual_data;
	int32_t *self_data;
    int 	mul_num;
    ...
    if (hx83102_touch_data->diag_cmd >= 1 && hx83102_touch_data->diag_cmd <= 7)
    {
        //Check event stack CRC
        if(!hx83102_diag_check_sum(hx83102_touch_data))
        {
            goto bypass_checksum_failed_packet;
        }
#ifdef HX_TP_PROC_2T2R
        if(hx83102_Is_2T2R && (hx83102_touch_data->diag_cmd >= 4 && hx83102_touch_data->diag_cmd <= 6))
        {
            mutual_data = hx83102_getMutualBuffer_2();
            self_data 	= hx83102_getSelfBuffer();

            // initiallize the block number of mutual and self
            mul_num = hx83102_getXChannel_2() * hx83102_getYChannel_2();

#ifdef HX_EN_SEL_BUTTON
            self_num = hx83102_getXChannel_2() + hx83102_getYChannel_2() + hx83102_ic_data->HX_BT_NUM;
#else
            self_num = hx83102_getXChannel_2() + hx83102_getYChannel_2();
#endif
        }
        else
#endif
        {
            mutual_data = hx83102_getMutualBuffer();
            self_data 	= hx83102_getSelfBuffer();

            // initiallize the block number of mutual and self
            mul_num = hx83102_getXChannel() * hx83102_getYChannel();

#ifdef HX_EN_SEL_BUTTON
            self_num = hx83102_getXChannel() + hx83102_getYChannel() + hx83102_ic_data->HX_BT_NUM;
#else
            self_num = hx83102_getXChannel() + hx83102_getYChannel();
#endif
        }

        hx83102_diag_parse_raw_data(hx83102_touch_data,mul_num, self_num,hx83102_touch_data->diag_cmd, mutual_data,self_data);

    }
    else if (hx83102_touch_data->diag_cmd == 8)
    {
        memset(hx83102_diag_coor, 0x00, sizeof(hx83102_diag_coor));
        memcpy(&(hx83102_diag_coor[0]), &hx83102_touch_data->hx_coord_buf[0], hx83102_touch_data->touch_info_size);
    }
    //assign state info data
    memcpy(&(hx_state_info[0]), &hx83102_touch_data->hx_state_info[0], 2);
...
void hx83102_log_touch_data(uint8_t *buf,struct hx_83102_report_data *hx83102_touch_data)
{
    int loop_i = 0;
    int print_size = 0;

    if(!hx83102_touch_data->diag_cmd)
        print_size = hx83102_touch_data->touch_info_size;
    else
        print_size = hx83102_touch_data->touch_all_size;
...
void hx83102_log_touch_event(int x,int y,int w,int loop_i,uint8_t EN_NoiseFilter,int touched)
{
...
void hx83102_log_touch_int_devation(int touched)
{
...
void hx83102_log_touch_event_detail(struct hx_83102_ts_data *ts,int x,int y,int w,int loop_i,uint8_t EN_NoiseFilter,int touched,uint16_t old_finger)
{
...
static int himax_diag_seq_read(struct seq_file *s, void *v)
{
...
#ifdef HX_TP_PROC_2T2R
    if(hx83102_Is_2T2R &&(g_diag_command >= 4 && g_diag_command <= 6))
    {
...
#ifdef HX_EN_SEL_BUTTON
        seq_printf(s, "\n");
        for (loop_i = 0; loop_i < hx83102_ic_data->HX_BT_NUM; loop_i++)
            seq_printf(s, "%6d", diag_self[hx83102_ic_data->HX_RX_NUM + hx83102_ic_data->HX_TX_NUM + loop_i]);
#endif
        seq_printf(s, "ChannelEnd");
        seq_printf(s, "\n");
    }
#ifdef HX_TP_PROC_2T2R
    else if(hx83102_Is_2T2R && g_diag_command >= 4 && g_diag_command <= 6)
    {
        ...
        for (loop_i = 0; loop_i < HX_BT_NUM; loop_i++)
            seq_printf(s, "%4d", diag_self[hx83102_ic_data->hx83102_RX_NUM_2 + hx83102_ic_data->hx83102_TX_NUM_2 + loop_i]);
#endif
        seq_printf(s, "ChannelEnd");
        seq_printf(s, "\n");
    }
    ...
    else if (g_diag_command == 8)
    {
        for (loop_i = 0; loop_i < 128 ; loop_i++)
        {
            ...
            seq_printf(s, "%4x", hx83102_diag_coor[loop_i]);
            if ((loop_i % 16) == 15)
                seq_printf(s, "\n");
        }
    }
...
bool hx83102_DSRAM_Flag = false;

//DSRAM thread
void hx83102_ts_diag_func(void)
{
    int i=0, j=0;
    unsigned int index = 0;
    int total_size = hx83102_ic_data->HX_TX_NUM * hx83102_ic_data->HX_RX_NUM * 2;
    ...
    {
        mutual_data = hx83102_getMutualBuffer();
        mutual_data_new = hx83102_getMutualNewBuffer();
        mutual_data_old = hx83102_getMutualOldBuffer();
    }
    hx83102_get_DSRAM_data(hx83102_ts->client, info_data);

    index = 0;
    for (i = 0; i < hx83102_ic_data->HX_TX_NUM; i++)
    {
        for (j = 0; j < hx83102_ic_data->HX_RX_NUM; j++)
        {
			new_data = (((int8_t)info_data[index + 1]) << 8 | info_data[index]);
            if(dsram_type == 1 || dsram_type == 4)
            {
                mutual_data[i*hx83102_ic_data->HX_RX_NUM+j] = new_data;
            }
            else if(dsram_type == 2)
            {
                //Keep max data for 100 frame
                if(mutual_data[i * hx83102_ic_data->HX_RX_NUM + j] < new_data)
                    mutual_data[i * hx83102_ic_data->HX_RX_NUM + j] = new_data;
            }
            else if(dsram_type == 3)
            {
                //Cal data for [N]-[N-1] frame
                mutual_data_new[i * hx83102_ic_data->HX_RX_NUM + j] = new_data;
                mutual_data[i * hx83102_ic_data->HX_RX_NUM + j] = mutual_data_new[i * hx83102_ic_data->HX_RX_NUM + j] - mutual_data_old[i * hx83102_ic_data->HX_RX_NUM + j];
            }
            index += 2;
        }
    }
    ...
    {
        queue_delayed_work(hx83102_ts->himax_diag_wq, &hx83102_ts->himax_diag_delay_wrok, 1/10*HZ);
    }
    ...
    else if(dsram_type == 2)
    {
        if(diag_max_cnt > 100) //count for 100 frame
        {
            ...
            //=====================================
            hx83102_diag_register_set(hx83102_ts->client, 0x00);
        }
        else
        {
            queue_delayed_work(hx83102_ts->himax_diag_wq, &hx83102_ts->himax_diag_delay_wrok, 1/10*HZ);
...
static ssize_t himax_diag_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
    ...
    storage_type = hx83102_determin_diag_storage(g_diag_command);
    rawdata_type = hx83102_determin_diag_rawdata(g_diag_command);
    ...
    {
        I("%s,common\n",__func__);
        if(hx83102_DSRAM_Flag)
        {
            //1. Clear DSRAM flag
            hx83102_DSRAM_Flag = false;
            //2. Stop DSRAM thread
            cancel_delayed_work(&hx83102_ts->himax_diag_delay_wrok);
            //3. Enable ISR
            hx83102_int_enable(hx83102_ts->client->irq,1);

            /*(4) FW leave sram and return to event stack*/
            hx83102_return_event_stack(hx83102_ts->client);
        }

        if(g_switch_mode == 2)
        {
            hx83102_idle_mode(hx83102_ts->client,0);
            g_switch_mode = hx83102_switch_mode(hx83102_ts->client,0);
        }
        ...
        if(g_diag_command == 0x04)
        {
            ...
            command[0] = g_diag_command;
        hx83102_diag_register_set(hx83102_ts->client, command[0]);
    }
    else if (storage_type > 0 && storage_type < 8 && rawdata_type > 0 && rawdata_type < 8)
    {
        ...
        queue_delayed_work(hx83102_ts->himax_diag_wq, &hx83102_ts->himax_diag_delay_wrok, 2*HZ/100);

        I("%s: Start get raw data in DSRAM\n", __func__);
        if (storage_type == 4)
            msleep(6000);
        //3. Set DSRAM flag
        hx83102_DSRAM_Flag = true;
    ...
    else if(storage_type == 8)
    {
        ...
        hx83102_idle_mode(hx83102_ts->client,1);
        g_switch_mode = hx83102_switch_mode(hx83102_ts->client,1);
        if(g_switch_mode == 2)
        {
            if(rawdata_type == 1)
                command[0] = 0x09; //IIR
                                ...
                                size_t len, loff_t *pos)
{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
        ...
        else if (debug_level_cmd == 'd')
        {
            ...
            if (hx83102_ic_data->HX_INT_IS_EDGE)
            {
            ...
            if (hx83102_ts->protocol_type == PROTOCOL_TYPE_A)
            {
        ...
        {
            if(hx83102_read_i2c_status(hx83102_ts->client))
        ...
        {
            if(hx83102_read_ic_trigger_type(hx83102_ts->client) == 1) //Edgd = 1, Level = 0
                ret += snprintf(temp_buf + ret, len - ret, "IC Interrupt type is edge trigger.\n");
            else if(hx83102_read_ic_trigger_type(hx83102_ts->client) == 0)
                                 ...
                                 size_t len, loff_t *pos)
{
    ...
    if ( buf[0] == 'h') //handshaking
    {
        ...
        handshaking_result = hx83102_hand_shaking(hx83102_ts->client); //0:Running, 1:Stop, 2:I2C Fail
    ...
    else if (buf[0] == 't')
    {
        ...
        result = request_firmware(&fw, fileName, hx83102_ts->dev);
		if (result < 0) {
        ...
        {
        case 32:
			if (hx83102_fts_ctpm_fw_upgrade_with_sys_fs_32k(hx83102_ts->client,
			...
			break;
		case 60:
				if (hx83102_fts_ctpm_fw_upgrade_with_sys_fs_60k(hx83102_ts->client,
					(unsigned char *)fw->data, fw->size, false) == 0) {
						E("%s: TP upgrade error, line: %d\n", __func__, __LINE__);
						fw_update_complete = false;
				} else {
				...
				break;
		case 64:
			if (hx83102_fts_ctpm_fw_upgrade_with_sys_fs_64k(hx83102_ts->client,
				(unsigned char *)fw->data, fw->size, false) == 0) {
					E("%s: TP upgrade error, line: %d\n", __func__, __LINE__);
                    fw_update_complete = false;
			} else {
			...
			break;
		case 124:
				if (hx83102_fts_ctpm_fw_upgrade_with_sys_fs_124k(hx83102_ts->client,
					(unsigned char *)fw->data, fw->size, false) == 0) {
						E("%s: TP upgrade error, line: %d\n", __func__, __LINE__);
						fw_update_complete = false;
				} else {
				...
				break;
		case 128:
				if (hx83102_fts_ctpm_fw_upgrade_with_sys_fs_128k(hx83102_ts->client,
					(unsigned char *)fw->data, fw->size, false) == 0) {
						E("%s: TP upgrade error, line: %d\n", __func__, __LINE__);
						fw_update_complete = false;
				} else {
	...
	else if(buf[0] == 'p')
	{
		I("NOW debug echo r!\n");
		//himax_program_sram();
		hx83102_ts->himax_0f_update_wq = create_singlethread_workqueue("HMX_update_0f_reuqest_write");
		if (!hx83102_ts->himax_0f_update_wq) 
			E(" allocate syn_update_wq failed\n");
		
		INIT_DELAYED_WORK(&hx83102_ts->work_0f_update, hx83102_0f_operation);
		queue_delayed_work(hx83102_ts->himax_0f_update_wq, &hx83102_ts->work_0f_update, msecs_to_jiffies(100));
		return len;
	}
	else if(buf[0] == 'x')
	{
...
firmware_upgrade_done:
    ...
    queue_delayed_work(hx83102_ts->himax_chip_monitor_wq, &hx83102_ts->himax_chip_monitor, g_hx83102_chip_monitor_data->HX_POLLING_TIMES*HZ);
#endif
                                        ...
                                        size_t len, loff_t *pos)
{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
    	...
    	hx83102_cmd_set[0] = 0x01;
        if(hx83102_read_FW_status(hx83102_cmd_set, tmp_data) == NO_ERR)
        {
            ret += snprintf(temp_buf + ret, len - ret, "0x%02X%02X%02X%02X :\t",hx83102_cmd_set[5],hx83102_cmd_set[4],hx83102_cmd_set[3],hx83102_cmd_set[2]);
            for (loop_i = 0; loop_i < hx83102_cmd_set[1]; loop_i++)
            {
                ret += snprintf(temp_buf + ret, len - ret, "%5d\t", tmp_data[loop_i]);
            }
            ret += snprintf(temp_buf + ret, len - ret, "\n");
        }
		hx83102_cmd_set[0] = 0x02;
		if(hx83102_read_FW_status(hx83102_cmd_set, tmp_data) == NO_ERR)
        {
            for (loop_i = 0; loop_i < hx83102_cmd_set[1]; loop_i = loop_i + 2)
            {
                if ((loop_i % 16) == 0)
                    ret += snprintf(temp_buf + ret, len - ret, "0x%02X%02X%02X%02X :\t",
                    hx83102_cmd_set[5],hx83102_cmd_set[4],hx83102_cmd_set[3]+(((hx83102_cmd_set[2]+ loop_i)>>8)&0xFF), (hx83102_cmd_set[2] + loop_i)&0xFF);
                                        ...
                                        size_t len, loff_t *pos)
{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
        ...
        {
            if(hx83102_read_DD_status(hx83102_cmd_set, tmp_data) == NO_ERR)
            {
                for (loop_i = 0; loop_i < hx83102_cmd_set[0]; loop_i++)
                {
                    if ((loop_i % 8) == 0)
    ...
    if (buf_tmp[2] == 'x' && buf_tmp[6] == 'x' && buf_tmp[10] == 'x')
    {
        ...
        for (i = 3; i < 12; i = i + 4)
        {
            ...
            if (!kstrtoul(buf_tmp2, 16, &result))
                hx83102_cmd_set[cnt] = (uint8_t)result;
            else
                I("String to oul is fail in cnt = %d, buf_tmp2 = %s",cnt, buf_tmp2);
            cnt--;
        }
        I("hx83102_cmd_set[2] = %02X, hx83102_cmd_set[1] = %02X, hx83102_cmd_set[0] = %02X\n",hx83102_cmd_set[2],hx83102_cmd_set[1],hx83102_cmd_set[0]);
    }
    else
        mutual_set_flag = 0;
                                      ...
                                      size_t len, loff_t *pos)
{
    ...
    else if (buf[0] == '1')// 1_32,1_60,1_64,1_24,1_28 for flash size 32k,60k,64k,124k,128k
    {
        ...
        queue_work(hx83102_ts->flash_wq, &hx83102_ts->flash_work);
    }
...
void hx83102_ts_flash_func(void)
{
    uint8_t local_flash_command = 0;
    ...
    {
        hx83102_flash_dump_func(hx83102_ts->client, local_flash_command,Flash_Size, flash_buffer);
    }
                                    ...
                                    size_t len, loff_t *pos)
{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
        ...
        hx83102_int_enable(hx83102_ts->client->irq,0);//disable irq
        g_hx83102_self_test_entered = 1;
        val = hx83102_chip_self_test(hx83102_ts->client);
#ifdef HX_ESD_RECOVERY
        HX83102_ESD_RESET_ACTIVATE = 1;
#endif
        hx83102_int_enable(hx83102_ts->client->irq,1);//enable irq
...
static ssize_t hx83102_sense_on_off_write(struct file *file, const char *buff,
                                        size_t len, loff_t *pos)
{
    char buf[80] = {0};
...
{
    .owner = THIS_MODULE,
    .write = hx83102_sense_on_off_write,
};
#endif
                                ...
                                size_t len, loff_t *pos)
{
    ...
    hx83102_set_HSEN_enable(ts->client, ts->HSEN_enable, ts->suspended);
...
int all_gesture_disable_flag = 0;	//zhaopengfei@wind-mobi.com 20180416 add
                                ...
                                size_t len, loff_t *pos)
{
    ...
    hx83102_set_SMWP_enable(ts->client, ts->SMWP_enable, ts->suspended);
                                  ...
                                  size_t len, loff_t *pos)
{
   ...
   // int i =0;
    ...
    if(!HX_PROC_SEND_FLAG)
    {
// zhaopengfei@wind-mobi.com 20180410 begin >>> [3/5] realize the touch panel gesture feature
		temp_buf = kzalloc(len,GFP_KERNEL);
...
*/
// zhaopengfei@wind-mobi.com 20180410 end >>> [3/5] realize the touch panel gesture feature
        if(copy_to_user(buf, temp_buf, len))
            I("%s,here:%d\n",__func__,__LINE__);
        kfree(temp_buf);
        HX_PROC_SEND_FLAG = 1;
                                  ...
                                  size_t len, loff_t *pos)
{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
        ...
        ret += snprintf(temp_buf + ret, len - ret, "EB_cnt = %d, EC_cnt = %d, ED_cnt = %d\n",hx83102_EB_event_flag, hx83102_EC_event_flag, hx83102_ED_event_flag);
        if(copy_to_user(buf, temp_buf, len))
            I("%s,here:%d\n",__func__,__LINE__);
        kfree(temp_buf);
        HX_PROC_SEND_FLAG=1;
                                   ...
                                   size_t len, loff_t *pos)
{
    ...
    if (buf[i] == '0')
    {
        hx83102_EB_event_flag = 0;
        hx83102_EC_event_flag = 0;
        hx83102_ED_event_flag = 0;
    }
                                      ...
                                      size_t len, loff_t *pos)
{
	...
	if (buf[0] == 'r')
    {
		I("%s,Test to get",__func__);
         queue_work(hx83102_ts->guest_info_wq, &hx83102_ts->guest_info_work);
...
int hx83102_touch_proc_init(void)
{
...
void hx83102_touch_proc_deinit(void)
{
